#!/usr/bin/env bash
#
# Runs Maestro test case 5 ("Enabled Activity tab modules are shown").
#
# Brings up the mock MediaWiki backend, optionally builds and installs the dev debug APK, then
# runs the parameter-set wrapper against a booted emulator.
#
#   .maestro/run-tc-5.sh                 # build + install + run
#   SKIP_BUILD=1 .maestro/run-tc-5.sh    # run only (app already installed)
#   FRESH_INSTALL=1 .maestro/run-tc-5.sh # uninstall first, so the flow really signs in
#
# `clearState` wipes app data but not the Wikipedia account in Android's AccountManager, so on a
# device that already ran this flow the sign-in step is skipped as a no-op (the flow still
# asserts the signed-in state). FRESH_INSTALL=1 removes the account by uninstalling the app.
#
...
if [[ -n "${FRESH_INSTALL:-}" ]]; then
  echo "==> Uninstalling org.wikipedia.dev (drops the leftover AccountManager session)"
  "$ADB" uninstall org.wikipedia.dev >/dev/null 2>&1 || true
fi
